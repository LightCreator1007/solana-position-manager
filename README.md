...
./install.sh          # installs to ~/.claude, use -y to skip the prompt
# or
./install-custom.sh   # choose personal or project-local
```
...
position-manager-skill/
  ...
  skill/
    ...
    engine/                 tested TypeScript core
      ...
      health.ts plan.ts safety.ts report.ts prices.ts
      sources/              per-venue readers, lazy SDK imports
      fixtures/             sample ledger and price series
      demo.ts
  agents/                   desk-analyst, rebalance-strategist, stream-sentinel
