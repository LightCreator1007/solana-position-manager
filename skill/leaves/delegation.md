...
1. Build the plan with `engine/plan.ts` `buildPlan(position, toBand, priceUsd)`. The plan lists the steps and a confirm phrase. No transaction is built here.
2. Pass each step to `solana-dev` to construct and simulate the transaction.
3. Run the plan through `engine/safety.ts` `guard(metrics, caps, ctx)`. It clears submission only when caps pass, the simulation succeeds, dry run is off, and the typed confirmation matches.
4. On a cleared guard, `solana-dev` signs and submits. On a rebalance that needs a token swap, route that leg to the Jupiter skill first.
